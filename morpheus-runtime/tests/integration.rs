...
fn test_escalation_gating() {
    ...
    scb.worker_state.store(WorkerState::Running as u32, Ordering::Release);
    scb.escalation_policy.store(EscalationPolicy::ThreadKick as u32, Ordering::Release);
    ...
    assert!(policy != EscalationPolicy::None, "Policy should allow escalation");
...
fn test_yield_acknowledgment() {
    ...
    // Check pending hints
    let pending = scb.preempt_seq.load(Ordering::Acquire) 
                > scb.last_ack_seq.load(Ordering::Acquire);
    ...
    scb.last_yield_reason.store(YieldReason::Hint as u32, Ordering::Release);
...
fn test_scheduler_mode_observer_only_default() {
    ...
    assert_eq!(mode, SchedulerMode::ObserverOnly, 
...
fn test_python_workers_not_escapable() {
    ...
    assert_eq!(
        python_scb.escapable.load(Ordering::Relaxed), 0,
...
fn test_rust_workers_escapable() {
    ...
    assert_eq!(
        rust_scb.escapable.load(Ordering::Relaxed), 1,
